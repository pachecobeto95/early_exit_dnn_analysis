...
def measuring_processing_time_acc(x, model, device, batch_size):
  processing_time_acc_list = []
  result = {"device": device, "batch_size": batch_size}  
  proc_acc = 0
  ...
  for i, exitBlock in enumerate(model.exits):
    ...
    proc_acc += processing_time
    result["tp_%s"%(i+1)] = proc_acc
  ...
  processing_time = time.time() - start
  proc_acc += processing_time
  result["tp_%s"%(len(model.exits) + 1)] = proc_acc
  return result

def measuring_processing_time(x, model, device, batch_size):
  ...
  for i, exitBlock in enumerate(model.exits):
    ...
    conf, infered_class = torch.max(model.softmax(output_branch), 1)
    processing_time = time.time() - start
    result["tp_%s"%(i+1)] = processing_time
  ...
  conf, infered_class = torch.max(model.softmax(output), 1)
  processing_time = time.time() - start
  result["tp_%s"%(len(model.exits) + 1)] = processing_time
  return result
...
#model_path = os.path.join(save_root_path, "%s_%s_%s_branches_%s.pth"%(model_name, dataset_name, model_id, n_branches)) #caltech
...
save_path = os.path.join(exp_dir_path, "results", "processing_time.csv")
save_acc_path = os.path.join(exp_dir_path, "results", "processing_time_acc.csv")
...
for batch_size in batch_size_list:
  for n in range(n_samples):
    print(n)
    x = torch.rand(batch_size, 3, input_dim, input_dim).to(device)
    processing_time_dict = measuring_processing_time(x, early_exit_model, device, batch_size)
    processing_time_acc_dict = measuring_processing_time_acc(x, early_exit_model, device, batch_size)
    df = df.append(pd.Series(processing_time_dict), ignore_index=True)
    df.to_csv(save_path)
    df.to_csv(save_acc_path)
